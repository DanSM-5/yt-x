...
function ui_prompt {
  # Only thread pipeline input when the caller actually pipes something (e.g. a
  # prefill for the rofi prompt / the download-title prompt). When called WITHOUT
  # input (interactive prompts like search), do NOT build a pipeline: otherwise the
  # native prompt tool (`gum input`) inherits an empty/EOF stdin and returns
  # instantly instead of waiting for the user — which made search run with an empty
  # query (0 results). See $MyInvocation.ExpectingInput.
  if ($MyInvocation.ExpectingInput) { $input | _ui_prompt @args } else { _ui_prompt @args }
}
...
function main {
  ...
  # Native tools (fzf, yt-dlp, …) emit UTF-8, but Windows consoles default to an OEM
  # code page (e.g. ibm437), so PowerShell mis-DECODES their output and mangles any
  # Unicode in menu items — e.g. the "New search…" ellipsis came back as "ΓÇª", which
  # then failed a string comparison (looked like a query) and got saved to history.
  # Force UTF-8 for console I/O. `[Console]::*` is process-global, so snapshot +
  # restore it (otherwise it would leak into the caller's session); `$OutputEncoding`
  # is script-scoped and disappears with the run.
  $__enc_out = [Console]::OutputEncoding
  $__enc_in = [Console]::InputEncoding
  try {
    [Console]::OutputEncoding = [Text.UTF8Encoding]::new($false)
    [Console]::InputEncoding = [Text.UTF8Encoding]::new($false)
  } catch { }
  $script:OutputEncoding = [Text.UTF8Encoding]::new($false)
  ...
  try {
    ...
    # restore the process-global console encodings changed above
    try { [Console]::OutputEncoding = $__enc_out; [Console]::InputEncoding = $__enc_in } catch { }
